...
class Adanet(Model):
    def __init__(self, name, loss_fn, input_dim, B):
        ...
        self.in_dims = [
            input_dim,
            B,
        ]  # stores the inputs dims for each layer in the network
    ...
    def add_subnetwork(self, sub_net):  # returns and appends the subnet to the network
        _network = [
            nn.Parameter(torch.cat((layer.weight, sub_layer.weight), 1))
...
class SubNetwork:
    ...
    def getA(self):
        sub_net = [
            nn.Parameter(torch.normal(_dim, self.B)) for _dim in self.in_dims[:-1]
        ]
        return sub_net
